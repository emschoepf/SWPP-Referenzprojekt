\title{Der Titel der Arbeit}

\begin{document}

\subject{Diplomarbeit}   %% subject which
% appears above titlehead

\subtitle{Untertitel der Arbeit}

% author(s)
%\author{Maximilian Hikel \and Emanuel Götsch}

\titlehead{
	\begin{minipage}[b]{0.64\textwidth}
	%\vspace{-30mm}
	%\hspace{-4mm}
	\includegraphics[height=3cm]{Logo_Projektpartner} % logo
	\end{minipage}
	\begin{minipage}[b]{0.35\textwidth}
	%\vspace{-32mm}
	\begin{flushright}
	\includegraphics[height=3cm]{Logo_HAK_Imst} % logo
	\end{flushright}
	\end{minipage}
	%\\
	%\centerline{\hrulefill}
}%end of titlehead

%\titlehead{Tierphysiologische Übungen Teil Dr. Hans Moser 
%\vspace{5mm}
%\includegraphics*{Bilder/UniLogo} % kleines logo
%}

\date{Imst, \today} 

\publishers{	
	\footnotesize
	eingereicht von:
	\begin{tabular}{ l c r }
	  Maximilian Hikel & Verantwortlich für IT: HTML, CSS, Angestellter \\
	  Emanuel Götsch & Verantwortlich für IT: HTML, CSS, Angestellter \\
	  \\\\
	\end{tabular}
	
	eingereicht bei: \\
	Dominik Neuner \\
	
}

